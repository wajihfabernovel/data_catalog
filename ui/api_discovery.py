...
def render_api_discovery(
    ...
    if not fetch_all:
        ...
        st.info(
            f"Will fetch custom entity names, keep only `{_PREFIX}*`, then run 5 API calls per matching table "
            f"(base attrs · lookup · string · picklist · decimal). "
    ...
    if run_fetch:
        ...
        if fetch_all:
            with st.status(f"Fetching '{_PREFIX}*' Dataverse tables…", expanded=True) as s:
                st.write(
                    f"Calling `EntityDefinitions?$select=LogicalName&$filter=IsCustomEntity eq true`, "
                    f"then filtering to `{_PREFIX}*` locally …"
                )
                try:
